...
export class PhysicsManager {
  ...
  buildWalls(): void {
    ...
    // Close the top and bottom of each net. The old goal only had a back
    // wall, letting the ball escape through the sides after a scored goal.
    const netSpan = FIELD.goalDepth + t / 2;
    addWall(FIELD_ORIGIN.x - netSpan / 2, midY - goalHalf, netSpan, 4);
    addWall(FIELD_ORIGIN.x - netSpan / 2, midY + goalHalf, netSpan, 4);
    addWall(FIELD_ORIGIN.x + FIELD.width + netSpan / 2, midY - goalHalf, netSpan, 4);
    addWall(FIELD_ORIGIN.x + FIELD.width + netSpan / 2, midY + goalHalf, netSpan, 4);
  }
